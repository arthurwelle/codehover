...
Since version 2.0.0 you no longer need to save each image by hand, nor set up any CSS/JavaScript. Give `ch_hover()` your ggplot code and it does the rest: the code is split at every top-level `+`, each partial plot is rendered to an image, and the result is a self-contained HTML object that works in R Markdown, Quarto and the RStudio viewer.
...
Since v2.0.0 `ch_out()` already returns a renderable object with the CSS and JavaScript attached — you no longer pass it through `htmltools::HTML()`, and no template is needed.

Inside `text` you can use `<br>` for line breaks, `&nbsp;`/`&ensp;`/`&emsp;` for spaces, and the tags `<tab1>` ... `<tab16>` for indentation levels. By default images are embedded into the HTML as base64 (self-contained single file); pass `url = TRUE` to reference images hosted elsewhere.
